...
import { Card, Button, Typography, Space, Input, Select, Radio, InputNumber, Upload, Steps, message } from 'antd';
import { ArrowLeftOutlined, PlusOutlined, UploadOutlined, CheckCircleOutlined } from '@ant-design/icons';
...
export function CampaignCreateScreen() {
  ...
  const [creativeOption, setCreativeOption] = useState<CreativeOption>('own');
      ...
      <Space direction="vertical" size={20} style={{ width: '100%' }}>
        ...
        {currentStep === 0 && (
          <Card>
            <Space direction="vertical" size={16} style={{ width: '100%' }}>
              <div>
                <Text strong style={{ display: 'block', marginBottom: '8px' }}>Название кампании</Text>
                <Input 
                  size="large"
              ...
              <div>
                <Text strong style={{ display: 'block', marginBottom: '8px' }}>Бюджет кампании (USDT)</Text>
                <InputNumber
                  size="large"
                  style={{ width: '100%' }}
                  min={10}
              ...
              <div>
                <Text strong style={{ display: 'block', marginBottom: '8px' }}>Длительность</Text>
                <Select
                  size="large"
                  style={{ width: '100%' }}
                  defaultValue="7"
              ...
              <Space direction="vertical" size={16} style={{ width: '100%' }}>
                <div>
                  ...
                  <Radio.Group 
                    ...
                    onChange={(e) => setCreativeOption(e.target.value)}
                    style={{ width: '100%' }}
                  >
              ...
              <Card>
                <Space direction="vertical" size={16} style={{ width: '100%' }}>
                  ...
                  <Text type="secondary" style={{ fontSize: '13px' }}>
                    💡 Рекомендуемые размеры: 1080x1080 для квадрата, 1920x1080 для видео
                  </Text>
                </Space>
              </Card>
            )}
            ...
            {creativeOption === 'order' && (
              <Card style={{ background: 'rgba(22, 119, 255, 0.05)' }}>
                <Space direction="vertical" size={16} style={{ width: '100%' }}>
                  ...
                  <div>
                    <Text strong style={{ display: 'block', marginBottom: '8px' }}>Тип креатива</Text>
                    <Select
                      size="large"
                      style={{ width: '100%' }}
                  ...
                  <div>
                    <Text strong style={{ display: 'block', marginBottom: '8px' }}>Техническое задание</Text>
                    <TextArea
                      rows={6}
                  ...
                  <Card size="small" style={{ background: '#fff' }}>
                    <Space direction="vertical" size={8} style={{ width: '100%' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <Text strong style={{ fontSize: '16px' }}>50 USDT</Text>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <Text type="secondary" style={{ fontSize: '13px' }}>
                    💡 После оплаты с вами свяжется менеджер контент-отдела для уточнения деталей
                  </Text>
                </Space>
              </Card>
            )}
        ...
        {currentStep === 2 && (
          <Card>
            <Space direction="vertical" size={16} style={{ width: '100%' }}>
              <div>
                <Text strong style={{ display: 'block', marginBottom: '8px' }}>Целевая аудитория</Text>
                <Select
                  size="large"
                  mode="multiple"
                  style={{ width: '100%' }}
              ...
              <div>
                <Text strong style={{ display: 'block', marginBottom: '8px' }}>География</Text>
                <Select
                  size="large"
                  mode="multiple"
                  style={{ width: '100%' }}
              ...
              <div>
                <Text strong style={{ display: 'block', marginBottom: '8px' }}>Язык контента</Text>
                <Select
                  size="large"
                  style={{ width: '100%' }}
                  defaultValue="ru"
        ...
        {currentStep === 3 && (
          <>
            <Card>
              <Space direction="vertical" size={16} style={{ width: '100%' }}>
                <div>
                  <Text type="secondary" style={{ fontSize: '13px' }}>Название кампании</Text>
                  <Title level={5} style={{ margin: '4px 0', fontWeight: 600 }}>{campaignName || 'Без названия'}</Title>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <Text type="secondary">Бюджет:</Text>
                  <Text strong style={{ fontSize: '16px' }}>{budget} USDT</Text>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <Text type="secondary">Креативы:</Text>
                  <Text strong>{creativeOption === 'own' ? 'Свои' : 'Заказ у контент-отдела'}</Text>
                </div>
                {creativeOption === 'order' && (
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Text type="secondary">Стоимость креативов:</Text>
                    <Text strong style={{ fontSize: '16px', color: '#faad14' }}>+50 USDT</Text>
                  </div>
                )}
              </Space>
            </Card>
            ...
            <Card style={{ background: 'rgba(22, 119, 255, 0.05)' }}>
              <Space direction="vertical" size={12} style={{ width: '100%' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <Text strong style={{ fontSize: '16px' }}>Итого к оплате:</Text>
            ...
            <Card>
              <Space direction="vertical" size={12} style={{ width: '100%' }}>
                ...
                <ol style={{ margin: 0, paddingLeft: '20px' }}>
                  <li><Text type="secondary">Кампания отправится на модерацию (1-2 часа)</Text></li>
                  <li><Text type="secondary">Средства заморозятся на вашем балансе</Text></li>
                  {creativeOption === 'order' && (
                    <li><Text type="secondary">Контент-отдел начнет работу над креативами</Text></li>
                  )}
                  <li><Text type="secondary">После одобрения кампания запустится автоматически</Text></li>
                </ol>
              </Space>
            </Card>
          </>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          ...
          {currentStep < 3 ? (
            <Button 
              ...
              onClick={() => setCurrentStep(currentStep + 1)}
              style={{ flex: 1 }}
              disabled={currentStep === 0 && !campaignName}
            >
              Далее
            </Button>
          ) : (
